...
class PluginSettingsActivity : ComponentActivity() {
  ...
  private fun detectHostPlatform(): String? {
    val packageNames = listOfNotNull(callingActivity?.packageName, referrer?.host).map(String::lowercase)
    return when {
      packageNames.any { it.contains("macrodroid") } -> "macrodroid"
      packageNames.any { it.contains("tasker") } -> "tasker"
      else -> null
    }
  }
...
private fun PluginSettingsContent(
  ...
  Column(Modifier.padding(20.dp).verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(10.dp)) {
    ...
    OutlinedTextField("%answer", {}, Modifier.fillMaxWidth(), label = { Text("出力変数") }, supportingText = { Text("成功: %answer / %used_model / %success、エラー内容: %error_message") }, singleLine = true, readOnly = true)
    Text("結果: %answer（回答）・%used_model（使用モデル）・%success（成否）・%error_message（エラー内容）")
