...
class LocaleFireReceiver : BroadcastReceiver() {
  override fun onReceive(context: Context, intent: Intent) {
    ...
    val executionId = intent.getStringExtra("com.example.ollamataskerbridge.REQUEST_ID").orEmpty()
    ...
    try {
      ...
      scheduleFallbackJob(context, intent, values)
    }
  }

  private fun scheduleFallbackJob(context: Context, original: Intent, values: android.os.Bundle?) {
    ...
    val result = context.getSystemService(JobScheduler::class.java).schedule(
      JobInfo.Builder(JOB_ID, ComponentName(context, InferenceJobService::class.java))
