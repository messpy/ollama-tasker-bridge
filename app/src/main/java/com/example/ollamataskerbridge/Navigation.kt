...
@Composable
private fun MacroDroidGuideScreen(modifier: Modifier = Modifier, onOpenDrawer: () -> Unit) { Column(modifier.padding(20.dp).verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(12.dp)) { androidx.compose.material3.TextButton(onClick = onOpenDrawer) { Text("☰ MacroDroid") }; Text("MacroDroid連携ガイド", style = MaterialTheme.typography.headlineSmall); Text("このアプリのLLMアクションをMacroDroidから実行できます。") ; Text("1. プラグイン設定") ; Text("MacroDroidでプラグインアクションを追加し、モデル・プロンプト・実行先を設定します。設定画面ではMacroDroidを選択してください。") ; Text("2. 入力に使う変数", style = MaterialTheme.typography.titleMedium); Text("プロンプト: %prompt\nモデル名: %model\n例: %prompt を「こんにちは」に置き換えて送信") ; Text("モデル欄には固定名（gemma3など）またはMacroDroid変数を指定できます。") ; Text("3. 受け取れる結果", style = MaterialTheme.typography.titleMedium); Text("回答: %answer\n使用モデル: %used_model\n成功: %success（true / false）\nエラー内容: %error_message") ; Text("次のMacroDroidアクションでは、{lv=answer}（回答）、{lv=used_model}（使用モデル）、{lv=success}（成功可否）、{lv=error_message}（エラー内容）を使用できます。") ; Text("4. 実行設定", style = MaterialTheme.typography.titleMedium); Text("プラグインアクションの「完了までブロック」をオンにし、タイムアウトは120秒以上にしてください。大きなローカルモデルではさらに時間がかかる場合があります。") } }
