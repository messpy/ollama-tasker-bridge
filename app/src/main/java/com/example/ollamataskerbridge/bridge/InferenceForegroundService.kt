...
class InferenceForegroundService : Service() {
    ...
    scope.launch {
      try {
        ...
        if (intent?.getStringExtra(EXTRA_ORIGIN) == ORIGIN_LOCALE) {
          val variables = Bundle().apply { putString("%error_message", message); putString("%used_model", intent?.getBundleExtra(LocalePluginContract.EXTRA_BUNDLE)?.getString(LocalePluginContract.KEY_MODEL).orEmpty()); putString("%success", "false") };
  ...
  private suspend fun runLocale(intent: Intent) {
    ...
    TaskerPlugin.addVariableBundle(extras, Bundle().apply { putString("%answer", result); putString("%used_model", model); putString("%success", "true") })
    val variables = Bundle().apply { putString("%answer", result); putString("%used_model", model); putString("%success", "true") };
